...
def call_dental_predict(con, database, schema, stage, table, build):
    '''
    This method calls predict function for the mentioned build on dental health claim 1 custom trained model
    ...
    :param build: Dental health claim 1 model build
    ...
    try:
        ...
        SELECT
            MD5 AS MD5_HASH,
            DENTAL_HEALTH_CLAIMS_1!PREDICT(GET_PRESIGNED_URL('@{stage}', RELATIVE_PATH), {build}) AS JSON_CONTENT,
            LAST_MODIFIED,
        ...
        if results.rowcount==0:
            log.warning("Dental health claim 1 predict did not return any results. Please check s3 stage for files.")
            ...
            log.info(f"Dental health claim 1 inserted {results.rowcount} records. Please check {schema}.{table}")
...
def main():
    ...
    if len(sys.argv) <= 1:
        ...
        call_dental_predict(con, "VP_"+env.upper()+"SLDB", "DOC_AI", env.upper()+"_ELDORADO_DENTAL", "LANDING_DENTAL_CLAIMS", model_build_version)
