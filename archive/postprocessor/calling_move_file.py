...
# usage:
# python calling_move_file.py -env=qa
# The script is calling 3 scripts in parallel.
# They do not impact each other, but if one fail the whole process calling_move_file will not be marked completed successfully.


DEFAULT_ROOT_DIR="/root/data-etl-pipelines"
SCRIPT_DIR=f'{DEFAULT_ROOT_DIR}/scripts/docai/eldorado/postprocessor/'
current_date = date.today().strftime("%Y%m%d")
log_file_path = "{DEFAULT_ROOT}/logs/{CURRENT_DATE}_calling_move_file.log".format(DEFAULT_ROOT=DEFAULT_ROOT_DIR,CURRENT_DATE=current_date)
...
def main():
    processes = []
    scripts = [
        [r'move_file.py',[f'-env={args.env}','-claim_type=dental']],
        [r'move_file.py',[f'-env={args.env}','-claim_type=institutional']],
        [r'move_file.py', [f'-env={args.env}','-claim_type=professional']]
    ]
